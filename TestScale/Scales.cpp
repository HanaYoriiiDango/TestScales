...
void block_world() {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        if (Hero.emotions[i] == 0 || Hero.emotions[i] == 100) { // Проверка только на эмоции текущего мира
        ...
        if (temp == "Start") { // начинаем диалог 
            ...
            while (start) {
                ...
                if (choice > 0 || choice < 2) { // проверяем чтобы ввод соответствовал количеству доступных игроку ответов 
                    ...
                    transition_control(); // применяем нашу функцию для того чтобы 
