...
void transition_control() { // эта функция нужна чтобы осуществлять перемещение игрока по шкалам из одного мира в другой

    Worlds_Num& c = Worlds_Num[round % 3];

    for (int i = 0; i < Worlds[Hero.current_loc].name.size(); i++) {

        if (Worlds[i].world_open) {

            Hero.current_loc = i;
            cout << "You transition: " << Worlds_Names[i] << endl;
        }
...
void block_world() {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        if (Hero.emotions[i] == 0 || Hero.emotions[i] == 100) { // Проверка каждой шкалы 

            Worlds[i].world_open = false;
            Worlds[i].portal[i].open = false;
